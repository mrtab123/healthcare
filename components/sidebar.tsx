...
export const Sidebar = async () => {
  ...
  return (
    <div className="w-full p-4 flex flex-col justify-between gap-4 bg-white overflow-y-scroll min-h-full remove-scrollbar">
      <div className="">
        ...
        <div className="mt-4 text-sm">
          {SIDEBAR_LINKS.map((el) => (
            <div key={el.label} className="flex flex-col gap-2">
              <span className="hidden uppercase lg:block text-gray-800 font-bold my-4">
                {el.label}
              </span>
              ...
              {el.links.map((link) => {
                if (link.access.includes(role.toLowerCase())) {
                  return (
                    <Link
                      href={link.href}
                      className="flex items-center justify-center lg:justify-start gap-4 text-gray-800 py-2 md:px-2 rounded-md hover:bg-blue-800 text-sm lg:text-base hover:text-white transition-all duration-200"
                      key={link.name}
                    >
